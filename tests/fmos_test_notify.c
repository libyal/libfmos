...
/* Tests the libfmos_notify_set_verbose function
 * Returns 1 if successful or 0 if not
 */
int fmos_test_notify_set_verbose(
     void )
{
...
/* Tests the libfmos_notify_set_stream function
 * Returns 1 if successful or 0 if not
 */
int fmos_test_notify_set_stream(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfmos_notify_set_stream(
	          NULL,
	          &error );
...
/* Tests the libfmos_notify_stream_open function
 * Returns 1 if successful or 0 if not
 */
int fmos_test_notify_stream_open(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
	...
	/* Test regular cases
	 */
	result = libfmos_notify_stream_open(
	          "notify_stream.log",
	          &error );
	...
	/* Clean up
	 */
	result = libfmos_notify_stream_close(
	          &error );
...
/* Tests the libfmos_notify_stream_close function
 * Returns 1 if successful or 0 if not
 */
int fmos_test_notify_stream_close(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FMOS_TEST_RUN(
	 "libfmos_notify_set_verbose",
	 fmos_test_notify_set_verbose )

	FMOS_TEST_RUN(
	 "libfmos_notify_set_stream",
	 fmos_test_notify_set_stream )

	FMOS_TEST_RUN(
	 "libfmos_notify_stream_open",
	 fmos_test_notify_stream_open )

	FMOS_TEST_RUN(
	 "libfmos_notify_stream_close",
	 fmos_test_notify_stream_close )
